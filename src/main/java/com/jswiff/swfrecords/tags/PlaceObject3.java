...
public final class PlaceObject3 extends Tag {
  ...
  protected void writeData(OutputBitStream outStream) throws IOException {
    ...
    //  stealth: NOT in a spec, but it seems the right place to go, see flasm code...
    if (cacheAsBitmap) {
    	 outStream.writeUI8(bitmapId);
    }
  ...
  private static Logger logger = Logger.getLogger(PlaceObject3.class);
  
  void setData(byte[] data) throws IOException {
    InputBitStream inStream = new InputBitStream(data);
    
    int start = inStream.available();
    logger.debug("DMP: "+UnsignedByte.dump(data));
    
    hasClipActions      = inStream.readBooleanBit();
    logger.debug("hasClipActions="+hasClipActions);
    hasClipDepth        = inStream.readBooleanBit();
    logger.debug("hasClipDepth="+hasClipDepth);
    hasName             = inStream.readBooleanBit();
    logger.debug("hasName="+hasName);
    hasRatio            = inStream.readBooleanBit();
    logger.debug("hasRatio="+hasRatio);
    hasColorTransform   = inStream.readBooleanBit();
    logger.debug("hasColorTransform="+hasColorTransform);
    hasMatrix           = inStream.readBooleanBit();
    logger.debug("hasMatrix="+hasMatrix);
    hasCharacter        = inStream.readBooleanBit();
    logger.debug("hasCharacter="+hasCharacter);
    ...
    logger.debug("cacheAsBitmap="+cacheAsBitmap);
    hasBlendMode    = inStream.readBooleanBit();
    logger.debug("hasBlendMode="+hasBlendMode);
    hasFilters      = inStream.readBooleanBit();
    logger.debug("hasFilters="+hasFilters);;
    ...
    // stealth: NOT in a spec, but it seems the right place to go, see flasm code...
    if (cacheAsBitmap) {
    	bitmapId = inStream.readUI8();
    }
    int end = inStream.available();
    logger.debug("#setData() inStream end:"+UnsignedByte.hex(end));
    logger.debug("DMP: "+UnsignedByte.dump(data,start-end));

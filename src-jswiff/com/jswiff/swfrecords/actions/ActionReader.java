...
public class ActionReader {
	...
	private static Logger logger = Logger.getLogger(ActionReader.class);
	...
	public static Action readRecord(InputBitStream stream) throws IOException {
		Action action;
		...
		// logger.debug("actionCode = "+actionFormat.sprintf(actionCode)+"
		// ("+ActionConstants.getActionName(actionCode)+"),length="+length);

//		if (length > 0) {
		boolean invalidActionLength = false;
		if (length > 0) {
			if (stream.available()>=length) {
				...
				logger.debug("Action "+actionCode+". stream.available() = "+stream.available()+", length="+length);
				invalidActionLength = true;
				InvalidAction invalidAction = new InvalidAction();
				invalidAction.setOffset(offset);
		...
		case ActionConstants.JUMP:
			Jump jmpAction = new Jump(actionStream);
			action = jmpAction;
			// logger.debug("jmpAction.getBranchOffset() =
			// "+jmpAction.getBranchOffset());
			// if (jmpAction.getBranchOffset()>0) {
			// logger.debug("Skipping next "+jmpAction.getBranchOffset()+" bytes
			// as unknown action");
			// }
			// stream.s
